...
public:
    ...
    void execute(const MemoryArgs& memory) override {
        if (m_dynQuant) {
            // The combined scratch lives in a per-NUMA-node, growable pool shared by every node
            // using DnnlScratchPad (see updateDynQuantScratch()). Another node's updateMemory()
            // can run between *this* node's own updateMemory() (where m_qsrcMem/m_srcScaleMem/
            // m_mmScratchMem's data handles were last set) and this execute() call - OV's dynamic
            // graph execution runs a prepare/update pass over *all* nodes before executing *any*
            // of them - and if that other node needs a bigger scratch, the shared block is
            // reallocated, invalidating the base pointer cached here. Refresh it from
            // m_combinedScratch (which always reflects the pool's current buffer) right before
            // use rather than relying on the possibly-stale handles set at update time.
            refreshDynQuantScratchHandles();
            quantizeSrc(memory);
        } else {
...
private:
    ...
    // Re-applies the *current* combined-scratch base pointer to the qsrc/src_scale/mm-scratch
    // dnnl::memory handles. dnnl::memory is a thin shared handle around the underlying C object,
    // so this also updates the same objects already referenced by m_primArgs - no need to
    // re-populate m_primArgs itself. Must be called from execute(), see its comment for why the
    // handles set during updateMemory() cannot be trusted to still be valid.
    void refreshDynQuantScratchHandles() {
        const size_t sizeQsrc = rnd_up(m_qsrcDnnlDesc.get_size(), 64);
        const size_t sizeScale = rnd_up(m_srcScaleDnnlDesc.get_size(), 64);
        auto* base = static_cast<uint8_t*>(m_combinedScratch->getData());
        m_qsrcMem.set_data_handle(base);
