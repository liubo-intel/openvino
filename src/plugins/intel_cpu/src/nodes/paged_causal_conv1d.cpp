...
void PagedCausalConv1D::initSupportedPrimitiveDescriptors() {
    ...
    // Natively accept f32/f16/bf16 on the float-typed ports. input_embeds/conv_weight/conv_bias
    // (ports 0/2/3) share the same precision and must match the output precision, so that the
    // kernel can treat them uniformly as DataT. conv_state_table (port 1) is independent; the
    // kernel converts its elements to/from f32 during accumulation and write-back.
    ...
    OPENVINO_ASSERT(
        ...
        data_precision);
    for (size_t port : {size_t{2}, size_t{3}}) {
        OPENVINO_ASSERT(getOriginalInputPrecisionAtPort(port) == data_precision,
                        "PagedCausalConv1D requires conv_weight (port 2) and conv_bias (port 3) to share the same "
                        "precision as input_embeds (port 0). Got port ",
                        port,
                        "=",
                        getOriginalInputPrecisionAtPort(port),
                        ", port 0=",
                        data_precision,
                        ".");
    }
